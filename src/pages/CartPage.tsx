...
export default function CartPage() {
  ...
  return (
    <div className="min-h-screen w-full bg-[#f7f7f5] flex flex-col overflow-x-hidden">
      ...
      <main className="flex-1 flex flex-col w-full px-8 md:px-16 lg:px-24 py-12 md:py-16">
        <div className="mb-10">
          ...
          <div className="flex flex-col lg:flex-row gap-8 flex-1">
            ...
            <div className="flex-1">
              <div className="bg-white border border-gray-100 rounded-lg overflow-hidden divide-y divide-gray-50">
                {items.map((item) => (
                  <div key={item.product.id} className="flex gap-6 p-6 md:p-8 hover:bg-gray-50/50 transition-colors duration-200">
                    <div className="w-24 h-24 md:w-28 md:h-28 rounded-md overflow-hidden bg-gray-50 flex-shrink-0">
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-[10px] text-gray-300 tracking-[0.2em] uppercase mb-1">{item.product.design?.name} · Talla {item.product.size?.label}</p>
                      <h3 className="text-gray-700 font-medium text-lg mb-2" style={{ fontFamily: 'Playfair Display, serif' }}>{item.product.name}</h3>
                      <p className="text-gray-400 text-sm">₡{Number(item.product.price).toLocaleString("es-CR")} c/u</p>
                    </div>
                    <div className="flex flex-col items-end justify-between">
                      <div className="flex items-center gap-4 border border-gray-200 rounded-sm px-4 py-2">
                      ...
                      <p className="text-gray-700 font-medium text-lg">₡{(Number(item.product.price) * item.quantity).toLocaleString("es-CR")}</p>
                      <button onClick={() => removeItem(item.product.id)}
                        className="text-red-400 hover:text-red-600 text-sm font-medium tracking-wide transition-all duration-200 hover:scale-105 px-2 py-1 rounded hover:bg-red-50">
                        Eliminar
                      </button>
                    </div>
                  </div>
            ...
            {/* Summary - shifted left, right margin so scale animation has room */}
            <div className="lg:w-[400px] flex-shrink-0 lg:mr-16">
              <div className="bg-white border border-gray-100 rounded-lg p-8 lg:sticky lg:top-24">
                <h2 className="text-2xl font-medium text-gray-800 mb-6 pb-6 border-b border-gray-100"
                ...
                <div className="border-t border-gray-100 pt-5 mb-8">
                  ...
                  <div className="flex justify-between items-center">
                    ...
                    <span className="text-gray-800 font-semibold text-3xl" style={{ fontFamily: 'Playfair Display, serif' }}>
                ...
                <button onClick={handleCheckout} disabled={loading}
                  className="w-full py-5 bg-[#1e3a5f] text-white text-base font-medium tracking-wide rounded-md hover:bg-[#16304f] hover:shadow-lg hover:shadow-[#1e3a5f]/20 hover:scale-[1.02] active:scale-[0.98] transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed mb-4">
                  {loading ? "Procesando..." : "Confirmar compra"}
                </button>
                <button onClick={() => navigate("/catalog")}
                  className="w-full py-5 border border-gray-200 text-gray-400 text-base tracking-wide rounded-md hover:border-gray-300 hover:text-gray-600 hover:scale-[1.02] transition-all duration-200">
                  Seguir comprando
                </button>
              </div>
            </div>
