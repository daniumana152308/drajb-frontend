...
export default function ProductDetailPage() {
    ...
    return (
      <div className="min-h-screen w-full flex flex-col overflow-x-hidden">
        ...
        <div className="flex-1 flex items-center justify-center">
          <p className="text-blue-500/60 text-base font-semibold tracking-widest animate-pulse">Cargando...</p>
        </div>
        <Footer />
      </div>
    );
  ...
  return (
    <div className="min-h-screen w-full flex flex-col overflow-x-hidden">
      ...
      <main className="flex-1 w-full px-8 sm:px-12 md:px-16 lg:px-24 py-6 sm:py-8">

        {/* Breadcrumb */}
        <div className="flex items-center gap-2 mb-6 sm:mb-8 text-sm text-gray-500 tracking-wide font-semibold">
          <button onClick={() => navigate("/catalog")}
            className="hover:text-blue-600 hover:scale-105 transition-all duration-200 min-h-[44px] flex items-center">
            Catálogo
          </button>
        ...
        <div className="flex flex-col lg:flex-row gap-8 sm:gap-10 lg:gap-16 items-start">
          ...
          <div className="w-full lg:w-2/5 flex-shrink-0">
            <div className="w-full bg-white border border-gray-200 overflow-hidden rounded-2xl flex items-center justify-center h-64 sm:h-80 lg:h-[440px] shadow-sm">
              {design?.imageUrl ? (
                <img
                  src={design.imageUrl}
                  alt={design.name}
                ...
                <div className="w-full h-full flex items-center justify-center bg-gradient-to-br from-blue-50 to-orange-50">
                  <span className="text-gray-400 text-sm tracking-widest uppercase font-semibold">Sin imagen</span>
                </div>
              )}
            </div>
          </div>
          ...
          <div className="flex-1 flex flex-col">
            ...
            <div className="mb-6">
              <div className="flex items-center justify-between mb-3">
                <p className="text-xs text-gray-500 tracking-[0.25em] uppercase font-bold">Talla</p>
              </div>
                  ...
                  <button
                    ...
                    className={`w-14 h-14 rounded-xl text-sm font-black tracking-wider transition-all duration-200 hover:scale-110 active:scale-95 border-2 ${
                      selectedProduct?.id === p.id
                        ...
                        : "border-gray-200 bg-white text-gray-600 hover:border-blue-400 hover:text-blue-600 hover:bg-blue-50"
                    }`}>
                    {p.size?.label}
                  </button>
                ))}
              ...
              {selectedProduct && (
                <p className="text-xs text-gray-400 mt-3 font-medium">{selectedProduct.size?.description}</p>
              )}
            </div>
            ...
            <div className="flex flex-col sm:flex-row gap-3 mb-6">
              ...
              <button onClick={() => navigate("/cart")}
                className="flex-1 py-4 min-h-[44px] border-2 border-gray-200 text-gray-600 text-sm font-bold tracking-widest rounded-xl hover:border-blue-400 hover:text-blue-600 hover:bg-blue-50 hover:scale-[1.02] transition-all duration-200">
                Ver carrito
              </button>
            </div>
              ...
              <div className="flex flex-col gap-3">
                {[
                  ...
                  <div key={label} className="flex gap-6 text-sm">
                    <span className="text-gray-400 w-36 flex-shrink-0 text-xs tracking-wide font-semibold">{label}</span>
