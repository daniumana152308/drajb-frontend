        ...
        <div className="min-h-screen w-full bg-[#f7f7f5] flex flex-col overflow-x-hidden">
            ...
            <main className="flex-1 flex flex-col">
                ...
                <section className="bg-[#1e3a5f] w-full relative overflow-hidden">
                    ...
                    <div className="relative w-full px-8 md:px-16 lg:px-24 py-14 sm:py-24 md:py-36">
                        <div className="max-w-3xl">
                            <p className="text-white/40 text-xs tracking-[0.3em] uppercase mb-4 sm:mb-6">Bienvenido de vuelta</p>
                            <h1 className="text-white leading-tight mb-6 sm:mb-8 select-none"
                                style={{ fontFamily: 'Playfair Display, serif', fontSize: 'clamp(2rem, 6vw, 5rem)', fontWeight: 600 }}>
                                Hola,<br />
                                <span className="italic font-medium text-white/70">{user?.firstName} {user?.lastName}</span>
                            </h1>
                            <p className="text-white/40 text-base sm:text-lg leading-relaxed mb-8 sm:mb-12 max-w-xl">
                            ...
                            <div className="flex flex-col sm:flex-row flex-wrap gap-4 sm:gap-5">
                                <button onClick={() => navigate("/catalog")}
                                    className="px-8 sm:px-12 py-4 sm:py-5 min-h-[44px] bg-white text-[#1e3a5f] text-base font-medium tracking-wide rounded-md hover:bg-gray-50 hover:shadow-xl hover:shadow-black/15 hover:scale-105 active:scale-[0.98] transition-all duration-200">
                                    Ver colección
                                </button>
                                <button onClick={() => navigate("/cart")}
                                    className="px-8 sm:px-12 py-4 sm:py-5 min-h-[44px] border-2 border-white/30 text-white text-base font-medium tracking-wide rounded-md hover:border-white hover:text-white hover:scale-105 active:scale-[0.98] transition-all duration-200">
                                    Mi carrito
                                </button>
                            </div>
                        </div>
                ...
                <section className="w-full px-8 md:px-16 lg:px-32 pt-14 pb-12 sm:pt-20 sm:pb-16 md:pt-28 md:pb-20">
                    <div className="mb-10 sm:mb-14">
                        ...
                        <h2 className="text-2xl sm:text-3xl md:text-4xl font-medium text-gray-800" style={{ fontFamily: 'Playfair Display, serif' }}>
                            ¿A dónde quieres ir?
                        </h2>
                    </div>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-5 sm:gap-8 md:gap-10">
                        {quickLinks.map((link, i) => (
                            <div key={link.title}
                                className="bg-white border border-gray-100 rounded-lg p-6 sm:p-8 md:p-10 group cursor-pointer hover:border-[#1e3a5f]/30 hover:shadow-xl hover:shadow-[#1e3a5f]/8 hover:scale-[1.02] transition-all duration-300"
                                onClick={link.action}>
                                <p className="text-[10px] text-gray-300 tracking-[0.25em] uppercase mb-5">0{i + 1}</p>
                                <h3 className="text-gray-800 font-medium text-xl sm:text-2xl mb-3" style={{ fontFamily: 'Playfair Display, serif' }}>
                                    {link.title}
                                </h3>
                                <p className="text-gray-400 text-sm leading-relaxed mb-6 sm:mb-8">{link.desc}</p>
